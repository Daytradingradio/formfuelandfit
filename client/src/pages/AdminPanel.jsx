...
const SAMPLE_WORKOUT = {
  ...
  days: [
    {
      day_number: 1, day_name: 'Day 1', focus: 'Chest & Triceps',
      notes: 'Focus on mind-muscle connection. Control the negative on every rep.',
      exercises: [
        { name: 'Barbell Bench Press', sets: 4, reps: '6-8', rest_seconds: 120, weight_note: 'Heavy — push hard' },
        { name: 'Incline Dumbbell Press', sets: 3, reps: '8-12', rest_seconds: 90, weight_note: 'Moderate' },
        { name: 'Cable Chest Flye', sets: 3, reps: '12-15', rest_seconds: 60, weight_note: 'Light-moderate' },
        { name: 'Tricep Pushdown', sets: 3, reps: '12-15', rest_seconds: 60, weight_note: '' },
        { name: 'Overhead Tricep Extension', sets: 3, reps: '10-12', rest_seconds: 60, weight_note: '' },
      ]
    },
    {
      day_number: 2, day_name: 'Day 2', focus: 'Back & Biceps',
      notes: 'Pull with your elbows, not your hands. Feel every rep in your back.',
      exercises: [
        { name: 'Pull-Ups', sets: 4, reps: '6-10', rest_seconds: 120, weight_note: 'Add weight if easy' },
    ...
    {
      day_number: 3, day_name: 'Day 3', focus: 'Rest / Active Recovery',
      notes: 'Light walk, stretching, or 20 min low-intensity cardio. Let your body recover.',
      exercises: []
    },
    {
      day_number: 4, day_name: 'Day 4', focus: 'Legs',
      notes: 'Legs day — no skipping. This is where the most growth happens.',
      exercises: [
        { name: 'Barbell Back Squat', sets: 4, reps: '6-8', rest_seconds: 180, weight_note: 'Heavy — go deep' },
        { name: 'Romanian Deadlift', sets: 3, reps: '8-10', rest_seconds: 120, weight_note: 'Feel the hamstring stretch' },
        { name: 'Leg Press', sets: 3, reps: '10-12', rest_seconds: 90, weight_note: '' },
        { name: 'Leg Curl', sets: 3, reps: '12', rest_seconds: 60, weight_note: '' },
        { name: 'Calf Raise', sets: 4, reps: '15-20', rest_seconds: 45, weight_note: '' },
      ]
    },
    {
      day_number: 5, day_name: 'Day 5', focus: 'Shoulders & Abs',
      notes: 'Control every shoulder rep. No momentum — strict form builds the most size.',
      exercises: [
        { name: 'Overhead Press', sets: 4, reps: '6-8', rest_seconds: 120, weight_note: 'Heavy' },
        { name: 'Lateral Raise', sets: 4, reps: '12-15', rest_seconds: 60, weight_note: 'Light — strict form' },
        { name: 'Front Raise', sets: 3, reps: '12', rest_seconds: 60, weight_note: '' },
...
const SAMPLE_NUTRITION = {
  ...
  carbs_g: 310,
  fats_g: 75,
  notes: 'Eat within 1 hour of waking. Have your biggest carb meals around your workout. Stay hydrated — minimum 1 gallon of water per day.',
  meals: [
    { meal_name: 'Breakfast', meal_time: '7:00 AM', calories: 550, protein_g: 40, carbs_g: 60, fats_g: 15,
      foods: ['5 whole eggs scrambled', '1 cup oatmeal with berries', '1 banana', 'Black coffee'] },
    { meal_name: 'Mid-Morning Snack', meal_time: '10:00 AM', calories: 350, protein_g: 35, carbs_g: 30, fats_g: 8,
      foods: ['Greek yogurt (plain, full fat)', '1 scoop whey protein', 'Handful of almonds'] },
    { meal_name: 'Lunch', meal_time: '1:00 PM', calories: 650, protein_g: 50, carbs_g: 70, fats_g: 15,
      foods: ['8oz grilled chicken breast', '1.5 cups white rice', 'Broccoli & green beans', 'Olive oil drizzle'] },
    { meal_name: 'Pre-Workout', meal_time: '3:30 PM', calories: 400, protein_g: 30, carbs_g: 55, fats_g: 5,
      foods: ['1 scoop whey protein', '1 large apple', '1 rice cake with honey', 'Creatine (5g)'] },
    { meal_name: 'Post-Workout', meal_time: '6:00 PM', calories: 500, protein_g: 45, carbs_g: 55, fats_g: 8,
      foods: ['8oz lean ground beef or salmon', '1 cup sweet potato', 'Side salad with vinaigrette'] },
    { meal_name: 'Dinner', meal_time: '8:00 PM', calories: 350, protein_g: 35, carbs_g: 25, fats_g: 12,
      foods: ['8oz sirloin steak or tuna', 'Steamed vegetables', '1/2 cup quinoa or brown rice'] },
    { meal_name: 'Evening Snack', meal_time: '10:00 PM', calories: 200, protein_g: 25, carbs_g: 15, fats_g: 5,
      foods: ['1 cup cottage cheese', 'Handful of walnuts', 'Casein protein shake (optional)'] },
  ]
}
...
export default function AdminPanel() {
  ...
  const savePlan = async () => {
    ...
    setSaving(false)
    setMsg(res.success ? '✓ Workout plan saved!' : 'Error saving plan')
  ...
  const saveNutrition = async () => {
    ...
    setSaving(false)
    setMsg(res.success ? '✓ Nutrition plan saved!' : 'Error saving nutrition plan')
    setTimeout(() => setMsg(''), 3000)
  }
    ...
    <div style={s.wrap}>
      <aside style={s.sidebar}>
        ...
        <nav style={s.nav}>
          {[
            ['members', '👥', 'Members'],
            ['messages', '💬', `Messages${threads.filter(t => t.unread_count > 0).length > 0 ? ` 🔴` : ''}`],
            ['videos', '🎬', 'Videos'],
          ].map(([t, icon, label]) => (
      ...
      <main style={s.main}>
              ...
              {members.map(m => (
                <div key={m.id} style={s.memberRow} onClick={() => selectMember(m)}>
                  ...
                  <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                    ...
                    <span style={{ ...s.statusDot, background: m.subscription_status === 'active' ? '#4caf7d' : '#555' }} title={m.subscription_status}></span>
                  </div>
                </div>
              ))}
        ...
        {tab === 'detail' && selectedDetail && (
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', margin: '2rem 0 1.5rem', borderBottom: '1px solid var(--border)', paddingBottom: '0' }}>
                  ...
                  {planForm.days.map((day, di) => (
                    <div key={di} style={s.dayBlock}>
                      ...
                      <div style={{ padding: '0.75rem 1rem' }}>
                        <input value={day.notes} onChange={e => updateDay(di, 'notes', e.target.value)} placeholder="Coach notes for this day..." style={{ width: '100%', marginBottom: '0.75rem', fontSize: '0.8rem' }} />
                        {day.exercises.map((ex, ei) => (
                          <div key={ei} style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem', alignItems: 'center' }}>
                            <input value={ex.name} onChange={e => updateEx(di, ei, 'name', e.target.value)} placeholder="Exercise" style={{ flex: 3 }} />
                            <input type="number" value={ex.sets} onChange={e => updateEx(di, ei, 'sets', e.target.value)} placeholder="Sets" style={{ flex: 1 }} />
                            <input value={ex.reps} onChange={e => updateEx(di, ei, 'reps', e.target.value)} placeholder="Reps" style={{ flex: 1 }} />
                            <input value={ex.weight_note} onChange={e => updateEx(di, ei, 'weight_note', e.target.value)} placeholder="Weight note" style={{ flex: 2 }} />
                            <button onClick={() => removeEx(di, ei)} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: '1rem', padding: '0.25rem' }}>✕</button>
                          </div>
                        ))}
                        <button onClick={() => addExercise(di)} style={{ ...s.navBtn, color: '#c9a84c', fontSize: '0.8rem', padding: '0.4rem 0' }}>+ Add Exercise</button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ marginTop: '1.5rem' }}>
                  ...
                  {nutritionForm.meals.map((meal, mi) => (
                    <div key={mi} style={s.dayBlock}>
                      <div style={s.dayBlockHeader}>
                        <input value={meal.meal_name} onChange={e => { const meals = [...nutritionForm.meals]; meals[mi].meal_name = e.target.value; setNutritionForm(f => ({ ...f, meals })) }} style={{ flex: 1, background: 'transparent', border: 'none', color: 'var(--white)', fontWeight: 500, outline: 'none' }} />
                        <input value={meal.meal_time} onChange={e => { const meals = [...nutritionForm.meals]; meals[mi].meal_time = e.target.value; setNutritionForm(f => ({ ...f, meals })) }} placeholder="Time" style={{ flex: 1, background: 'var(--surface2)', fontSize: '0.8rem' }} />
                        <input type="number" value={meal.calories} onChange={e => { const meals = [...nutritionForm.meals]; meals[mi].calories = e.target.value; setNutritionForm(f => ({ ...f, meals })) }} placeholder="kcal" style={{ flex: 1, background: 'var(--surface2)', fontSize: '0.8rem' }} />
                        <input type="number" value={meal.protein_g} onChange={e => { const meals = [...nutritionForm.meals]; meals[mi].protein_g = e.target.value; setNutritionForm(f => ({ ...f, meals })) }} placeholder="protein g" style={{ flex: 1, background: 'var(--surface2)', fontSize: '0.8rem' }} />
                      </div>
                      <div style={{ padding: '0.75rem 1rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {videos.map(v => (
                <div key={v.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: '1rem 1.25rem', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <button onClick={() => deleteVideo(v.id)} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: '0.8rem', padding: '0.25rem 0.5rem' }}>Delete</button>
                </div>
              ))}
            </div>
          </div>
